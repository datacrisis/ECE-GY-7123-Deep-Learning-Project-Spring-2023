    ...
    def __init__(self, opt, device, type='train', downscale=1, n_test=10):
        ...
        if self.mode == 'colmap':
            ...
            # only load one specified split
                ...
                with open(os.path.join(self.root_path, f'transforms_train.json'), 'r') as f: ###HOTWIRING
                    print("Hotwiring to transforms_train.json on VAL set in  NeRFDataset")
                    transform = json.load(f)
...
#            print(len(frames))
            ...
            for f in tqdm.tqdm(frames, desc=f'Loading {type} data'):
                f_path = os.path.join(os.getcwd(), f['file_path'])
...
#                print(f,f['transform_matrix'])
                pose = np.array(f['transform_matrix'], dtype=np.float32) # [4, 4]
#                print("PRE NERF MATRIX TO NGP", pose)
                pose = nerf_matrix_to_ngp(pose, scale=self.scale, offset=self.offset)
#                print("POST NERF MATRIX TO NGP", pose)
